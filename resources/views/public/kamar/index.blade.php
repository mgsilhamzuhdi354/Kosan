<x-public-layout title="Daftar Kamar">
    <section class="app-home-shell">
        <div class="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between gap-3">
                <div>
                    <p class="text-xs font-black uppercase tracking-[0.18em] text-sky-700">Cari Kos</p>
                    <h1 class="mt-1 text-2xl font-black tracking-tight sm:text-4xl">Kamar tersedia untuk kamu</h1>
                    <p class="mt-1 text-sm font-semibold text-slate-500">{{ $kamars->total() }} kamar ditemukan</p>
                </div>
                <a href="https://wa.me/6283179749407" target="_blank" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 shadow-sm"><x-icon name="message" class="h-5 w-5" /></a>
            </div>

            <form method="GET" class="app-search-panel mt-5 overflow-hidden rounded-[1.75rem]">
            ...
            <div class="mobile-safe-scroll mt-4 flex gap-2 overflow-x-auto pb-2">
                <a href="{{ route('public.kamar.index') }}" class="filter-chip {{ request()->query() ? '' : 'filter-chip-active' }}">Urutkan</a>
                <a href="{{ route('public.kamar.index', array_merge(request()->except('promo'), ['promo' => 1])) }}" class="filter-chip {{ request('promo') ? 'filter-chip-active' : '' }}">Promo</a>
                <a href="{{ route('public.kamar.index', array_merge(request()->except('harga_max'), ['harga_max' => 1500000])) }}" class="filter-chip {{ request('harga_max') ? 'filter-chip-active' : '' }}">Harga</a>
                @foreach ($tipes->take(3) as $tipe)
                    <a href="{{ route('public.kamar.index', array_merge(request()->except('tipe'), ['tipe' => $tipe])) }}" class="filter-chip {{ request('tipe') === $tipe ? 'filter-chip-active' : '' }}">{{ $tipe }}</a>
                @endforeach
                <a href="{{ route('public.kamar.index', array_merge(request()->except('status'), ['status' => 'tersedia'])) }}" class="filter-chip {{ request('status') === 'tersedia' ? 'filter-chip-active' : '' }}">Status</a>
            </div>
        </div>
    </section>
            ...
            @forelse ($kamars as $kamar)
                <article class="compact-room-card overflow-hidden rounded-3xl">
                    <div class="grid grid-cols-[132px_1fr] sm:grid-cols-[220px_1fr]">
                        <a href="{{ route('public.kamar.show', $kamar) }}" class="relative min-h-44 sm:min-h-56">
                                ...
                                <div class="flex items-start justify-between gap-2">
                                    ...
                                    @auth
                                        @if (auth()->user()->isPenyewa())
                                            <form method="POST" action="{{ route('penyewa.favorit.store', $kamar) }}">
                                                @csrf
                                                <button class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-50 text-slate-500"><x-icon name="heart" class="h-4 w-4" /></button>
                                            </form>
                                        @else
                                        ...
                                        @endif
                                    @else
                                        <a href="{{ route('login') }}" class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-50 text-slate-500"><x-icon name="heart" class="h-4 w-4" /></a>
                                    @endauth
                                </div>
