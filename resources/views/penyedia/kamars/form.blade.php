<x-penyedia-layout :header="$kamar->exists ? 'Edit Kamar' : 'Tambah Kamar'">
    <form method="POST" action="{{ $kamar->exists ? route('penyedia.kamar.update', $kamar) : route('penyedia.kamar.store') }}" enctype="multipart/form-data" class="grid gap-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:grid-cols-2">
        @csrf
        ...
        <div>
            ...
            <input name="nama_kamar" value="{{ old('nama_kamar', $kamar->nama_kamar) }}" class="mt-1 w-full rounded-2xl border-slate-300" required>
        </div>
        <div>
            <label class="text-sm font-bold">Tipe Kamar</label>
            <input name="tipe_kamar" value="{{ old('tipe_kamar', $kamar->tipe_kamar) }}" class="mt-1 w-full rounded-2xl border-slate-300" required>
        </div>
        <div>
            <label class="text-sm font-bold">Harga Bulanan</label>
            <input name="harga_bulanan" type="number" min="1" value="{{ old('harga_bulanan', $kamar->harga_bulanan) }}" class="mt-1 w-full rounded-2xl border-slate-300" required>
        </div>
        <div>
        ...
        <div class="lg:col-span-2">
            ...
            <textarea name="deskripsi" rows="4" class="mt-1 w-full rounded-2xl border-slate-300" required>{{ old('deskripsi', $kamar->deskripsi) }}</textarea>
        </div>
        <div class="lg:col-span-2">
            <label class="text-sm font-bold">Fasilitas</label>
            <div class="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                @foreach ($fasilitas as $item)
                    <label class="flex items-center gap-2 rounded-2xl bg-slate-50 p-2 text-sm">
        ...
        <div class="flex gap-3 lg:col-span-2">
            ...
            <a href="{{ route('penyedia.kamar.index') }}" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-black">Batal</a>
        </div>
    </form>
</x-penyedia-layout>
